...
- [ ] T043 [US5] Implement the bus-fault logic in `link/health.cpp` per data-model.md §7 (declare rule, alternation in `next_probe`, clear-and-pin on the first valid answer *(amended 2026-09-13, F4: a reference-rate answer clears at once; a fallback answer starts a reference pass of every enrolled address, `BusState.fallback_answerer` (the address to enrol on the clear, 0 = none)/`ref_pass_left`; no automatic return — data-model §7)*, `BusStats`), citing `trunk §7` — make T041 and T042 pass
