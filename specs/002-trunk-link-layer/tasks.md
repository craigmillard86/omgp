...
- [ ] T030 [US2] Implement the remaining `MockWire` kinds in `tests/support/mock_wire.cpp` — make T028 pass
  ...
  - Ruling 2026-09-03 (see issue #48): widen `Step::count` to `uint32_t` atomically in this task, all four artefacts together. This is a T3 slice by the ruling's own classification, but NO path-based gate labels it so (`risk-score` and CODEOWNERS cover neither `specs/**/contracts/` nor `tests/support/**`) — #48 carried `needs-human` until a human released it (review on PR #122)
