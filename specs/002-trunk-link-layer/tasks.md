...
- [ ] T030 [US2] Implement the remaining `MockWire` kinds in `tests/support/mock_wire.cpp` — make T028 pass
  - `CrcError`/`Duplicate` are implemented (PR #137, as T029's own slice of this task, per that PR's `tests/support/mock_wire.cpp` file comment); `Garbage`/`Babble`/`Rate` remain, per the ruling below — this task is NOT done
