...
MESSAGES = [
    ...
    msg("msg_bp_slot_map_req", "protocol-l3 §3.1 BP_SLOT_MAP (empty request)", G.OP_BP_SLOT_MAP, 0x02, 1, 0),
    msg("msg_bp_slot_map_resp", "protocol-l3 §3.1 BP_SLOT_MAP response (R-01)", G.OP_BP_SLOT_MAP, 0x02, 1, RESP,
        l3.BpSlotMapResp(4, bytes.fromhex("0A"), bytes.fromhex("01"))),
    msg("msg_bp_slot_map_resp_full_occupancy",
        "protocol-l3 §3.1 BP_SLOT_MAP response, slot_count at LIMIT_bp_slot_map_max_slots (R-01)",
        G.OP_BP_SLOT_MAP, 0x02, 12, RESP,
        l3.BpSlotMapResp(G.LIMIT_bp_slot_map_max_slots, b"\xff" * 29, b"\xff" * 29),
        note="boundary: slot_count at the cap, every occupied/changed bit set"),
