...
// "simulated time that does not advance"), not from link/master.{hpp,cpp} directly.
// link/master.{hpp,cpp} (PR #137 review) is a signature-only stub: T031, a separate issue,
// replaces every body there with the real retry/timing/acceptance state machine this file
// asserts against, so every TEST_CASE here is expected to fail its REQUIREs (not to fail to
// compile) until T031 lands. Every assertion is driven through the scripted MockWire +
...
TEST_CASE("a CRC-failed response ends the attempt immediately, without waiting out the "
          "remaining timeout",
          "[link]") {
    // Kind::CrcError's behaviour lands in T030 (tasks.md); link/master.{hpp,cpp} is a
    // signature-only stub until T031 (see the file header) and never calls
    // MockWire::transmit(), so this case goes red at the first assertion below rather than
    // exercising CrcError at all — expected per this issue's "out of scope" note, not a
    // defect in this test.
    FakeClock clock;
    MockWire wire(clock);
...
TEST_CASE("a late duplicate of a concluded transaction's response is discarded during the "
          ...
          "[link]") {
    // Kind::Duplicate's behaviour lands in T030; link/master.{hpp,cpp} is a signature-only
    // stub until T031 (see the file header) and never calls MockWire::transmit(), so this
    // case goes red at the first assertion below rather than exercising Duplicate at all -
    // same as the CRC-failure case above, expected, not a defect in this test.
    FakeClock clock;
    MockWire wire(clock);
    ...
    // before last_activity (== full_end0) + T_gap; script exhausted (Duplicate was a
...
TEST_CASE("a late response from a different, already-concluded destination (wrong src) "
    ...
    const uint64_t a_full_end = response_full_end(a_tx_end, dst, 0, payload, sizeof payload);
