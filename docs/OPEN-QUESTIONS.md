...
**Refined after the round-10 red team on #472:** at most one probe is outstanding at a time while a fault is declared — `next_probe()` returns "nothing to issue" (`addr = 0`) until the previous outcome arrives — so no transaction can be in flight when a pass starts and every outcome during the pass belongs to the pass probe; the round-9 matcher by address and rate could not be exact while two transactions to one address could exist, and needed a rate the contract's `on_result` does not carry. The contract's `Probe` gains the `addr = 0` meaning. Every pass probe is issued at the reference rate, so the pass bound is 30 ms whatever rate was in use before the fault.

**Refined after the round-9 red team on #472:** `ref_pass_left` is decremented only by the outcome of the probe the pass issued, never by a pre-pass transaction; and the rate-scaling of the superframe is keyed to the rate a transaction is *issued* at, since `bit_rate` is assigned only by a clear and every fault-time fallback probe is issued at a rate other than `bit_rate`.
...
**Where it lands:** `docs/trunk-link-layer.md` §7; `specs/002-trunk-link-layer/data-model.md` §6 (`next_probe()` during a fault and during the pass) and §7 (`BusState` gains `fallback_answerer` — the address to enrol on the clear, 0 = none — and `ref_pass_left`; the clear rule as above; implemented by T043); `spec.md` FR-026 (rewritten as one rule), the 2026-08-29 clarification and US5 acceptance scenario 3; `contracts/link-cpp.md` `bit_rate()` and the health Rules; `tasks.md` (US5 goal, T043) and `research.md`, which stated the superseded clear-and-pin rule. `protocol/omgp-protocol.yaml` is unchanged at this head (the two keys were added at `b85171d` and removed again). The remainder of #155 (F1, F2(b), F3) is untouched here.

**Supersedes:** the "re-probes on a fixed cadence … returns when a quorum answers" clauses of the 2026-09-06 F4 entry, which keeps its history; and this entry's own first revision at `b85171d`. **Amends:** the four artefacts named above, each with a dated marker.
