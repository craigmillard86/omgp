...
**What stands at the head of #530** (`3cd2bc0`): the exemption is per address (`HealthRecord::probe_issued_us`), bounded by the slowest transaction's whole outcome window — `2 × kMaxWire × byte_time_us(TRUNK_bit_rate_fallback) + TRUNK_T_resp_us`, ≈ 24.6 ms, spec symbols only — and never cancelled by a later handout; a repeated ok outcome for the recorded fallback answerer while the fault stands is a no-op.
