...
**Context:** this session ruled the 2026-09-07 Responder/SC-004 cluster and two governance entries: **11 pending entries, 8 decisions** once the supersede chains collapse. Each ruling is recorded here rather than by editing the entries (this file is append-only). Where an entry's own recommendation was changed or withdrawn, that is stated.

**This is NOT a sweep of the backlog, and an earlier revision of this entry wrongly said it was.** That revision claimed "every entry still marked PENDING or carrying no ruling line: 12 entries". The sweep behind it matched one spelling, `**Ruling:** PENDING`. Measured at this PR's base (`main`): **44 entries carry a pending ruling** — 11 upper-case `PENDING` and 33 lower-case `pending`, of which 27 read `pending — human`; the other six use different tails and at least four are already resolved in substance. (This PR appends one further `PENDING` entry, the T_gap tie-break below, so 45 at its head.) The 33 this session does not rule are untouched and remain open, several of them adjacent to what is ruled here — `"the held-request queue unbounded and uncounted; FR-014 and FR-017 conflict on a stale queued request"`, `"the `{peer, seq}` replay key lets one forged frame evict a pending station's replay entry"`, and `"RequestHandler's 'called at most once per NEW sequence' overclaims"`. Normalising the marker so the backlog is mechanically countable is filed separately.
...
2. **2026-09-05 "Route review findings by scope, not severity (#134)"** — **already ruled; recorded here only for completeness.** The entry carries "**Recommendation / ruling:** ADOPTED (human, via #134)", a combined field the sweep did not match, which is why an earlier revision of this entry wrongly called it unruled. `GOVERNANCE.md` records "ruled 2026-09-05". Nothing changes.
3. **2026-09-07 "SC-004: all 16 positions ARE exercised, by 15 cases…"** — **Ruling:** adopted in substance: no matrix position is uncovered and no duplicate case is required. **The count in the entry's title is wrong, and so was an earlier revision of this ruling.** Counted at this head, `tests/unit/test_link_loop.cpp` holds 16 `TEST_CASE`s across the four matrix rows (drop 3, delay-past-T_resp 5, CRC-corrupted 4, duplicate 4). Two of the delay row's cases occupy no matrix position — "at attempts 0 AND 1" and "at the boundary" — so **14 scripts cover the 16 positions**, two rows each sharing one script across the coinciding "retry 2"/"after give-up" columns at `TRUNK_retries == 2`. T034's description is amended to those numbers. The two earlier SC-004 entries it supersedes are closed with it.
...
5. **2026-09-07 "the Responder's late path CAN transmit into a frame it has not read"**, with "…defers for an idle bus", "…stops reading to avoid dropping" and the "retraction" entry — **Ruling:** the property preserved is **"the engine never keys down onto a bus it has not read, nor over another station's arriving frame"**, and it outranks FR-014's "MUST still transmit — at once" and data-model §5's "nothing is dropped". *(Corrected: an earlier revision of this ruling said "FR-017 and trunk §3 outrank …". That framing is wrong and the repo had already withdrawn it once, at the 2026-09-06 entry on the held-request queue: a late response is by construction outside its response window — that is what `late_responses` counts — so FR-017 is **not** the property this design preserves. FR-017 and trunk §3's "never transmit outside its response window" bullet remain unamended and in tension with FR-014's late-transmit clause. That tension is NOT ruled here; it is the still-open 2026-09-06 entry named above.)* The engine never stops reading during a late wait. A completed request beyond `kHeldRequests` is discarded and counted in `stats()`, so a late response keys down only on a bus the engine has read. Implementation: #372. FR-014 and data-model §5 carry ruling markers.
...
6. **2026-09-07 "does trunk §4's T_gap bind every station, or only the host between its own transactions?"** — the clause is trunk **§3**. **Ruling:** the gap binds every station, between transactions; within a transaction, `T_turn` governs the request→response turnaround. Trunk §3's bullet and the timing table are amended. Implementation: #374.
   - **Two limits, added after review.** (a) This does **not** license a node to transmit outside a response window: trunk §3's "a node transmits only in the response window" and FR-017 still forbid that, FR-014 is what requires the late transmit, and that conflict stays open. What the gap bounds is the instant at which such a transmission may key down, and §3 now names both cases: a node's own back-to-back transmissions, and the late response counted by `late_responses`. *(A first revision of the §3 amendment gave the late response as an example of a licensed transmission; a second excluded it altogether, leaving the node half of the clause binding nothing. Both are corrected.)* (b) The host and a node can otherwise become legal at the same microsecond on a bus §3 says has no arbitration, no CSMA and no token. The amendment therefore states that **the node yields**: the host is the sole initiator, so where both gaps expire together the node waits. That tie-break was not put to the maintainer and is the conservative reading, recorded so it is visible rather than assumed.
7. **2026-09-07 "the Responder's replay entry has no age bound…"** — **Ruling:** replay a retry only when it repeats the request that was answered, compared over **`dst`, `src`, `len` and the L3 payload**, with `ctrl` (which carries the retry bit) and the CRC **excluded**. `ReplayBuffer` gains a fixed-size copy of those bytes. Implementation: #373; data-model §5 marker.
   - **A non-matching retry is discarded, not treated as new** (added after the round-2 review). A frame with the retry bit set whose `seq` and `peer` match the buffer but whose compared bytes differ is discarded and counted: the handler is not invoked and the buffer is left intact. Treating it as new would let a spoofed frame — L2 has no authentication — evict the buffer and turn one logical `GET_EVENT` into two drains, widening the very eviction surface this ruling narrows and breaching golden rule 2.
   - **Which bytes, and why it matters.** An earlier revision said "request bytes" without qualification. In these documents "bytes" means the complete frame, and trunk §7 has the host retransmit with `ctrl.retry` set, so a retry is never byte-identical to the request it retries. Taken literally that rule would never match, every retry would re-enter the handler, and replay would be disabled — the very outcome for which this ruling withdraws the entry's own proposal, and a hit on golden rule 2, since a retried `GET_EVENT` would drain a second event.
...
**Still pending after this session:** **33 entries** whose ruling line is lower-case `pending` (27 of them `pending — human`), untouched by this session and listed by the marker-normalisation issue; plus 2026-09-11 "#340's AC 1 ('stop at a horizontal rule') conflicts with its AC 2", which is on PR #341's branch and is enacted when #341 merges.

**Supersedes:** none. This entry records rulings on the entries named, and each keeps its history.

---

## 2026-09-11 — T_gap tie-break: which station yields when the host's gap and a node's expire together?

**Context:** the 2026-09-11 ruling that `T_gap` binds every station (trunk §3, amended in PR #375) creates a case the document does not otherwise settle. Take `R1` = the end of a node's response. The host's earliest legal request is `R1 + T_gap`; a node's earliest legal next transmission is also `R1 + T_gap`. Trunk §3 states there is "no multi-master arbitration, no CSMA, no token", and the document contains no tie-break, priority or backoff rule. Both engines already compute the same expression from the same input (`link/master.cpp:129`, `link/responder.cpp:298`), so on a half-duplex RS-485 trunk both drivers key down together.

**Recommendation:** the node yields — the host is the sole initiator, so where both gaps expire at the same instant the node waits and the host transmits. The §3 amendment in #375 states this, marked as the conservative reading rather than a ruling.
...
**AC2, the bounded retry, is built.** `agent_retry_max` is **1**, #361's stated default: a run that produced nothing is retried once in-job before the claim is released or the attempt returned. Unreadable, absent, negative or non-numeric fails closed to 0; values above **1** are clamped down to the one retry step the workflows contain (a clamp of 2 advertised an attempt that was not spendable — round-2 review), the same shape as `auto_fix_max_attempts`.

The first version of this entry argued for shipping **0**, on the grounds that a retry step must duplicate the primary prompt verbatim (Actions has no YAML anchors) and that the observed no-ops died on permission denials, so the same prompt would hit the same wall. The round-1 review on #416 was right to reject that: #361 is a directive with explicit parameters, not a spec ambiguity with a safe default, so `PENDING — human` was the wrong route. And the duplication objection was answerable — both Claude steps now read ONE prompt from `.github/agent-prompts/`, the directory this repo already used for `continuous-improvement.md`, so there is no second copy to drift.

**Two further judgements, stated rather than buried:**
- *What counts as production* is deliberately generous: for dispatch, an open PR on `task/<issue>` created or pushed to by this run, or commits pushed to that branch with no PR (**partial**: kept and not retried, but reported and the job failed, since AC1 names "a pushed branch *and* an opened PR"); for review-fix, a moved head **or** the *fixer's own* comment made during the run — identified by the first line `review-fix(<attempt>) @ <head>` that `review-fix.md` step 5 now requires — because the fixer is explicitly allowed to rebut a finding and change no code. Other workflows post as `claude[bot]` on the same PR and are not the fixer. `VERDICT(...)` comments are excluded — claude-review and red-team post on the same PR, and one landing mid-run must not be mistaken for the fixer's work. A false no-op (a real PR abandoned, a claim released under a working agent) is worse than a missed one, which is merely today's behaviour.
- *No transcript artifact.* The first version uploaded the raw execution file for 7 days and called it "private to the repo". `GOVERNANCE.md` §4b says plainly that **this repository is public**, so the artifact was downloadable by anyone, and secret masking covers step logs rather than bytes inside an uploaded file — from a job holding `CLAUDE_CODE_OAUTH_TOKEN`. The upload is gone. The figures that actually diagnose a stall (turns, denials, `is_error`) go in the job log and in the comment `finalize` posts.

**Corrected after adversarial round 1 (@`44e58f7`).** Twelve findings between the review and the red team; all correct. Three were false claims in this entry and in the workflows' own comments, not gaps:

- *"Both guards were `if: failure()`, so neither could ever run."* False. Run `33332211854` shows the dispatch guard firing: the Claude step failed, and "Release claim if no PR was opened" ran and released the claim. The true, narrower claim is that it could not fire for a run that **exits success** having produced nothing — which is every one of the five. The distinction is load-bearing, because the old step also covered *failures*, and the first version of `finalize` returned silently when `detect` itself threw. An unknown outcome now fails closed and releases the claim, as the old step did.
- *"The artifact is private to the repo."* False, as above.
- *"`agent_retry_max` ships at 0, PENDING — human."* Superseded: AC2 is built, at the stated default of 1.

Also fixed: a missing execution file no longer un-establishes production that detection has already found (a cancelled run was declaring a no-op while its own `reason` said the head had moved, releasing a live claim); a `task/<n>` PR left open by an **earlier** dispatch no longer vouches for this run — the exact #58 → #401 shape, where the claim is released and the issue re-picked while the first PR is still open; the verdict exclusion now scans every line unwrapped from backticks and emphasis, because the repo writes verdicts backticked and review comments end with an attribution footer; `dropLabel` distinguishes "already absent" from "removal failed", so the comment can no longer assert that no claim was held when the WIP cap is in fact stuck; `finalize` reports `detect`'s actual reason instead of a hard-coded sentence that could contradict it; and the escalation comment can no longer be lost to a 403 on `needs-human`. The dispatch job now loads the detector from the **default branch**, as review-fix already did — the monitored agent runs first, holding `Edit`/`Write` on that workspace, so requiring the gate from there let it rewrite its own check.

Three mutants survived the first suite (the `SINCE` window, the `state: 'open'` filter, the fail-closed term); all three now have cases.

**Corrected after adversarial round 2 (@`c76c6ac`).** The clamp was lowered from 2 to 1, as above; `noop` and `produced` became separate outputs so a review-fix run that pushed and then errored is escalated but never retried; `updated_at` was replaced by the PR's last commit date as the push signal, because GitHub bumps `updated_at` on any comment or label; a rebuttal that *quotes* the verdict it rebuts is production, not a verdict; the detector and config are fetched from the default branch into `$RUNNER_TEMP`, outside the tree the agent edits and commits from.

**Corrected after adversarial round 3 (@`2559c21`).** Seven findings between the review and the red team, all in code this change introduced:

- *An unknown run start fell open.* `t0` had no `if:`, so a failed apt/pip/checkout step skipped it, `SINCE` arrived empty, and every window test passed: an older PR, a stale `task/<n>` branch or attempt 1's comment vouched for a run in which no agent ran, and the claim was held — where the replaced `if: failure()` step released it. This falsified this entry's own sentence "an unknown outcome now fails closed", which was true only when `detect` threw. Now `t0` is `always()` **and** the module treats an empty or unparseable `SINCE` as unknown: nothing is attributable, the claim is released, the job fails. A moved head still counts, since `HEAD_BEFORE` comes from the gate.
- *The retry could run with an empty prompt.* The same skipped prompt step left `steps.prompt.outputs.text` empty while the retry gate was true. Both retry steps are gated on the first Claude step not having been skipped.
- *Any `claude[bot]` comment vouched for the fixer.* The marker line, as recorded above. A verdict written as a list item or inside HTML tags now unwraps to a verdict; `<blockquote>` is a quote, like `>`.
- *An errored run that had produced output was handled as if it had produced nothing.* `finalize` handed `review-fix-<n>` back on a run that pushed code, and released `in-progress` under a PR its own `Detected:` line said was opened. `finalize` now reads `produced`: the attempt stays spent, the claim stays with the work, the headline matches the reason, the job still fails.
- *A branch pushed with no PR exited green.* Round 2 was right to keep the claim and wrong to go quiet: it is #58's exact shape. `partial`, as recorded above.
- *The detector-location pin asserted a string, not the property.* `require('./tools/ci/agent-noop.js')` satisfied "not `GITHUB_WORKSPACE`". The pin now requires the literal `RUNNER_TEMP` load and forbids `__dirname`, `process.cwd` and `./tools`.
- Two stale figures: the clamp text above, and the case count in the PR body.
...
**Refined after the round-3 red team on #523:** the round-1 statement "both engines key down at exactly `last activity + T_gap`" was true of the uncapped path only. Both engines also carry the bounded-courtesy cap (`defer_origin + max_frame + T_gap`), and where the cap binds the node keys down with less than `T_gap` of idle. *(Corrected at round 5, after #372 merged under this PR: the round-3 wording described a `want_us = cap_us` partial-drain path that #372 removed — the wait is now `min(last activity + T_gap, defer_origin + max_frame + T_gap)`, `last_activity_us_` consulted on every path, and the residual is recorded by the merged 2026-09-14 entry "the cap can still fire into a frame it HAS read" and #528, not by the 2026-09-07 entry.)* trunk §3 now states both paths; #527 covers the uncapped tie only and says so; the capped path stays with #528.
...
**Refined after the round-2 red team on #523:** three corrections to the round-1 wording. (1) Obligation 3 could not deliver "in step" while only `Master::set_bit_rate` had a refusal rule (`bps == 0`, `bps > 10 Mb/s`, `byte_time_us` would be 0; neither setter reports acceptance) — a refused argument changed the tracker and not the wire (demonstrated with a stub wire: `bps=0 → wire=1000000 tracker=0`). `HealthTracker::set_bit_rate` now carries the identical refusal rule, so a refused rate changes neither. (2) data-model §7's "Fall back" bullet said the declare rule was the only way off the reference rate; it now names the setter as the second path and states the consequence of using it on a live rig (enrolled nodes polled at a rate they cannot hear → SUSPECT → fault → cleared pinned at the fallback rate) as the layer above's choice, not a property the tracker guards. (3) The setter comment's "the rate enrolment probes are issued at" is qualified with "while `!bus_fault()`": during a fault the probe's rate is the probe's own (alternation, reference pass).

**Refined after the round-3 red team and review on #523:** the round-2 consequence chain was the opposite of what §7 produces — after the layer above drops a live reference-rate rig to the fallback rate, the nodes fall SUSPECT, the fault is declared, the alternating probes reach them at the reference rate and the first answer there clears the fault at the reference rate: the health rules undo the selection (an automatic return, at the cost of one fault cycle), so the "pinned at the fallback rate" outcome was unreachable. data-model §7 now says so. Two further corrections: the setter counts an accepted rate once in `BusState.rate_changes` (§7's "on each change of `bit_rate`") while `Master::set_bit_rate` counts the same action once in `BusStats.rate_changes` — two counters answering two questions, stated deliberately; and the setter's cases are written into T041 (tests first), with T043 adding them red-first if T041 has landed without them.
...
**What stands at the head of #530** (`ae74a62` and after; the `3cd2bc0` wording "a repeated ok outcome for the recorded fallback answerer is a no-op" was superseded by round 11): the exemption is per address (`HealthRecord::probe_issued_us`), bounded by the slowest transaction's whole outcome window — `2 × kMaxWire × byte_time_us(TRUNK_bit_rate_fallback) + TRUNK_T_resp_us`, ≈ 24.6 ms, spec symbols only — and never cancelled by a later handout; the same window bounds the reference pass's hold on its outstanding probe (round 14: unbounded, one pass probe whose outcome never arrived wedged the trunk for ever — the probe is written off as "drew nothing" after the window and the pass advances, clearing at the fallback rate if it was the last). Outside a fault, an ok outcome for a probe that went out at a rate other than the rate now in use — the late answer to a written-off pass probe after a fallback clear, or to an alternation probe after a reference clear — moves no §6 state while that probe is still outstanding inside its window (round 15: enrolling on it stranded the node as ENROLLED on a trunk it cannot hear, or polled it straight back into a re-declared fault; round 16: read from the last probe's rate bit alone the rule suppressed ordinary status-poll answers for ever, so it is bounded to a live probe inside its window, ok and failure alike). Past the window — which is where every written-off pass probe's late outcome lands, by construction — the outcome is indistinguishable from a fresh one and is read as one: a late reference-rate answer enrols the node on the fallback-rate trunk, and §6 returns it to SUSPECT within `TRUNK_suspect_after_failures` polls it cannot hear; a late failure counts. The clear is not undone. The same accepted cost as the window itself, in both directions. Every address that answered at the fallback rate in the episode is remembered (`BusState::fallback_seen`); an ok outcome from any of them is never a reference-rate clear, a §6 transition or a pass restart, but if that address's pass probe is outstanding it is that probe's outcome and advances the pass (round 11: a complete no-op stalled the pass for ever, and a single answerer field protected one address). **A second divergence from data-model §7, recorded here (round-12 review):** on a fallback-rate clear every address in that record takes its deferred §6 transition, where §7 says "the recorded answerer → ENROLLED". Recommended amendment to §7: "every address that answered at the fallback rate in this episode → ENROLLED" (each demonstrated it hears that rate; enrolling one and leaving the rest SUSPECT would have them status-polled at the rate now in use, which they hear, so it is a delay, not a hazard — but the record is what the code does). `BusState.fallback_answerer` stays as the data model's named field, written and not read.

**What this cannot do, stated (rule 11):** inside one outcome window an abandoned probe (outcome dropped at L2, abandoned at a superframe boundary, timeout never reported) is indistinguishable from an outstanding one, so its record is honoured for up to ≈ 24.6 ms and written off after. A reference-rate answer from that address arriving inside the window is read at the abandoned probe's fallback bit and deferred to a pass rather than clearing at once; the pass then probes it at the reference rate and clears there, one pass late. The round-7 reproducer's 10 ms abandonment reads that way by design. The alternatives each produced a demonstrated worse failure: a shorter window wrote off live probes (round 9), supersession dropped live records for a conforming scheduler (round 10), and no exemption at all inverted both clear rules (round 6).

**Recommendation:** accept the window as the only bound. Under the F3 obligations (probes are the only traffic during a fault; one `next_probe()` call per probe issued; every issued probe gets an `on_result`, answer or timeout, within its transaction's window) no probe is abandoned, and the cost above is reachable only through obligation slack. If a tighter bound is wanted, the honest way is a fourth obligation — F3 reports every issued probe's outcome — not a shorter timer.
