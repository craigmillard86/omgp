...
**Where it lands:** `docs/trunk-link-layer.md` §7; `specs/002-trunk-link-layer/data-model.md` §7 (`BusState` gains `fallback_answered` and `ref_pass_left`; the clear rule as above; implemented by T043); `spec.md` FR-026 and the 2026-08-29 clarification; `contracts/link-cpp.md` `bit_rate()` and the health Rules. `protocol/omgp-protocol.yaml` is unchanged at this head (the two keys were added at `b85171d` and removed again). The remainder of #155 (F1, F2(b), F3) is untouched here.
