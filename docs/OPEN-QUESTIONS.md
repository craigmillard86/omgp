...
6. **2026-09-07 "does trunk §4's T_gap bind every station, or only the host between its own transactions?"** — the clause is trunk **§3**. **Ruling:** the gap binds every station, between transactions; within a transaction, `T_turn` governs the request→response turnaround. Trunk §3's bullet and the timing table are amended. Implementation: #374.
   - **Two limits, added after review.** (a) This does **not** license a node to transmit outside a response window: trunk §3's "a node transmits only in the response window" and FR-017 still forbid that, and an earlier revision of the §3 amendment wrongly gave a held or late response as an example of a licensed transmission. What the gap bounds is the instant at which a transmission FR-014 *already requires* — the late path, counted by `late_responses` — may key down. (b) The host and a node can otherwise become legal at the same microsecond on a bus §3 says has no arbitration, no CSMA and no token. The amendment therefore states that **the node yields**: the host is the sole initiator, so where both gaps expire together the node waits. That tie-break was not put to the maintainer and is the conservative reading, recorded so it is visible rather than assumed.
...
**Still pending after this session:** **33 entries** whose ruling line reads `pending — human`, untouched by this session and listed by the marker-normalisation issue; plus 2026-09-11 "#340's AC 1 ('stop at a horizontal rule') conflicts with its AC 2", which is on PR #341's branch and is enacted when #341 merges.
