...
**Refined after the round-9 red team on #472:** the pass has one probe in flight at a time and `ref_pass_left` is decremented only by the outcome of the probe the pass issued (`BusState.pass_addr`), never by a pre-pass transaction still in flight when the pass started; and the rate-scaling of the superframe is keyed to the rate a transaction is *issued* at, since `bit_rate` is assigned only by a clear and every fault-time fallback probe is issued at a rate other than `bit_rate`.
