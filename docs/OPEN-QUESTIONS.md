...
**Refined after the round-9 red team on #472:** `ref_pass_left` is decremented only by pass-probe outcomes — which, since probes are the only traffic during a fault (round 11), are the only outcomes there are; and the rate-scaling of the superframe is keyed to the rate a transaction is *issued* at, since `bit_rate` is assigned only by a clear and every fault-time fallback probe is issued at a rate other than `bit_rate`.

**Refined after the round-10 red team and review on #472:** rounds 9 and 10 specified a matcher for outcomes arriving out of order (`pass_addr`, then `outstanding` and a "nothing to issue" probe). Trunk §3 forbids the premise: the trunk is a strict master poll with one transaction at a time, so the fallback answer that starts a pass is the outcome of the only transaction there was, and every outcome during the pass is the pass probe's. That machinery is withdrawn. Round 11 then showed trunk §3 alone was not enough either: `on_result` reports every transaction, and a demand-slot outcome landing mid-pass would have ended the pass early — so FR-026 now suspends demand traffic as well as status polls while a fault is declared (every enrolled node is SUSPECT or OFFLINE; nothing can be delivered), and with probes the only traffic the simple rule is sound. Every pass probe is issued at the reference rate; the pass bound is 30 ms on a trunk at the reference rate and ≈ 261 ms on a trunk already at the fallback rate, whose superframes are stretched although the probes are not.
