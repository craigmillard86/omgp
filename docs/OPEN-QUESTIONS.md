...
- **"Never lower" became mechanical.** It was previously protected only by `pipeline.sh` being owner-reviewed. `tools/refimpl/test_floor_datum.py` fails any decrease against the branch this targets, fetching that base explicitly and failing closed on CI when it cannot be obtained. Deleting that test scores T3 — `risk-score.yml`'s "reduces test content" rule now covers `tools/refimpl/test_*` as well as `^tests/` — so the ratchet cannot be removed on an autonomous merge either, and changing the floor datum scores T2 rather than T0.
