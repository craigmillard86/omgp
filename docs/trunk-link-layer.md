...
- Frames whose L3 node ID belongs to one of the backplane's slots are translated to module-bus transactions. The backplane MUST answer on the trunk within T_resp on every poll: with the module's reply if it is ready, otherwise `ERR_BUSY` (the host retries later). The backplane never holds a trunk response open waiting on the module bus (module-bus timeout 5 ms is an internal bound, invisible to trunk timing); it MUST NOT stall the trunk waiting on I2C. (Reworded 2026-09-03 per docs/OPEN-QUESTIONS.md — the previous phrasing read as sanctioning a wait.)
