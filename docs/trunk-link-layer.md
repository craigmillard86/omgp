...
- **Inter-frame gap**: ≥ **T_gap = 50 µs** of bus idle separates any two transactions, and the gap binds every station. The host leaves it before each new request, and a node leaves it before any transmission that does not answer the frame just received — for example a held or late response following its own previous response. Within a transaction, **T_turn** above governs the request→response turnaround. *(Amended 2026-09-11 by maintainer ruling; previously "the host leaves ≥ T_gap … between transactions". See `docs/OPEN-QUESTIONS.md` 2026-09-07 "does trunk §4's T_gap bind every station" and #374.)*
