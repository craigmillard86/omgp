...
- Frames whose L3 node ID belongs to one of the backplane's slots are translated to module-bus transactions. For every poll it accepts (a frame discarded under §4 gets no response, as for any node), the backplane MUST answer on the trunk within T_resp: with the module's reply if it is ready, otherwise `ERR_BUSY` (the host retries later). The module-bus transaction proceeds independently under its 5 ms module-bus timeout — answering `ERR_BUSY` never abandons or restarts it, and that timeout is invisible to trunk timing. An `ERR_BUSY` answer is a valid response at L2 — the poll itself did not fail; how *persistent* busy is bounded (a wedged bridge answering busy forever) is an open question, decided by neither this bullet nor §7 (docs/OPEN-QUESTIONS.md, 2026-09-03 persistent-`ERR_BUSY` entry). The backplane never holds a trunk response open waiting on the module bus; it MUST NOT stall the trunk waiting on I2C. (Reworded 2026-09-03 per docs/OPEN-QUESTIONS.md; forward-progress, §4 and §7 scoping added per red-team review on #122.)
